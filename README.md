...
因此，手在手机画面中向左、向右、向上、向下和手指张开的形状，会与 Blender 固定摄像机中看到的位置基本一致。`画面位置倍率` 默认为 `1.0`，通常不需要修改；只有希望刻意放大动作范围时才调整。
...
固定场景右侧新增一个“摇杆弹球”装置：底座固定不动，上方球体通过弹性杆连接。手击打球体时，球会产生物理冲量、摆动并自动回正，类似拳击球。
固定场景中包含四个交互物体：

- `TOGGLE`：用手的碰撞体积撞击，开关切换状态。
- `BOUNCE`：用手的碰撞体积撞击，按钮弹跳。
- `SPIN`：用手的碰撞体积撞击，球体获得旋转冲量。

手部由 21 个关节球体、20 个指节胶囊和掌心球体组成碰撞体积；物体使用球体或盒体碰撞体积。碰撞后按接触深度和手部速度计算冲量，物体受重力、落地反弹和物体间碰撞影响。左右手之间不进行碰撞。侧栏可调整 `击打力度阈值`、启用手部体积碰撞，或点击 `重置交互物体`。
